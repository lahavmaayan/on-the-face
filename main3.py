...
kMinSamplesPerPerson = 20
kTrainSamplesCount = 16
...
class train_ImageList(data.Dataset):
    def __init__(self, fileList, transform=None, train_loader=train_loader):
        # self.root      = root
    ...
    def __getitem__(self, index):
        ...
        # Read the image
        img = self.train_loader(os.path.join(imgPath))
...
class test_ImageList(data.Dataset):
    def __init__(self, fileList, transform=None, test_loader=test_loader):
        # self.root      = root
        self.imgList   = fileList
        self.transform = transform
    ...
    def __getitem__(self, index):
        ...
        # Read the image
        img = self.test_loader(os.path.join(imgPath))
...
def generate_sets(dir_path):
    ...
    for d in candidates:
        ...
        if len(file_list) >= kMinSamplesPerPerson:
            ...
            train_set.extend(((os.path.join(subdir_path, f), class_idx) for f in file_list_permute[:kTrainSamplesCount]))
            test_set.extend(((os.path.join(subdir_path, f), class_idx) for f in file_list_permute[kTrainSamplesCount:]))
...
def train(train_dataloader, forward_pass, criterion, optimizer, epoch):
    ...
    for i, data in enumerate(train_dataloader):
        ...
        print("Epoch: {}, current iter: {}/{}\n Current loss {}\n".format(epoch, i, len(train_dataloader), loss.item()))
...
def main():
    ...
    global classes
    train_set, test_set, classes = generate_sets(r'd:/deeplearning/data')
    ...
    train_dataloader = torch.utils.data.DataLoader(
                        train_ImageList(fileList=train_set,
    ...
    test_dataloader = torch.utils.data.DataLoader(
                        test_ImageList(fileList=test_set,
                                transform=transforms.Compose([
                                transforms.Scale((128,128)),
                                transforms.ToTensor(),            ])),
                        shuffle=True,
